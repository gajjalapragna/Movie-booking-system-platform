...
export const Seatselection = () => {
  ...
  const startSelect = e => {
    ...
    if (name === "" || ceats === "") {
      ...
      UserService.getSeatsbooked(moviename).then(res => {
        ...
        for (var i = 0; i < data.length; i++) {
          if (
            data[i].bookingdate == bookingdate &&
            data[i].theatername == theatername &&
            data[i].showtime == showtime
          ) {
            var bookedseats = data[i].seatnames;
            var array = bookedseats.split(",");
            for (var j = 0; j < array.length; j++) {
              $("#" + array[j]).attr("disabled", true);
            }
          } else {
            $("#" + data[i].seatnames).attr("disabled", false);
          }
        }
      });
    }
  ...
  const bookingFunction = e => {
    ...
    if (
      ...
      let data = {
        ...
        showtime: showtime,
        mobile: mobile,
        seatnames: allSeatarray.toString(),
      ...
      localStorage.setItem("moviewatchers", name);
      localStorage.setItem("seatnames", allSeatarray.toString());
      localStorage.setItem("totalcost", ticketcost * ceats);

      console.log("data => " + JSON.stringify(bookedseats));
      UserService.addBooking(data).then(res => {
        console.log(res);
        UserService.addBookedseats(bookedseats).then(res => {
          console.log(res);
            ...
            >
              ...
              <tr>
                <td style={{ color: "black" }}>
                  <textarea style={{ color: "black" }} id="nameDisplay" />
                </td>
                <td style={{ color: "black" }}>
                  <textarea style={{ color: "black" }} id="NumberDisplay" />
                </td>
                <td style={{ color: "black" }}>
                  <textarea style={{ color: "black" }} id="seatsDisplay" />
                </td>
              </tr>
            </table>
            <br />
